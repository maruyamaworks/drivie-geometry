...
import { parseMyTracksCSV, parseGPX, parseNMEA } from '../dist/index.js';

test('parseMyTracksCSV', async () => {
...
`;
  const result = parseMyTracksCSV(data);
...
<gpx creator="jp.drivie.recorder" version="1.1">
  <trk>
    <name>Shinjuku-ku, Tokyo, Japan</name>
    <trkseg>
...
`;
  const result = await parseGPX(data);
